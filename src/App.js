...
function PhotoListItem({ file }) {
  return (
    <li className='relative'>
      <div className='group aspect-square block w-full overflow-hidden rounded-lg bg-gray-100 transition-all duration-300 hover:scale-105 hover:shadow-lg'>
        <Image
          src={file.source}
          alt={file.title}
      ...
      <p className='pointer-events-none mt-2 block truncate text-sm font-semibold text-indigo-600'>
        {file.title}
      </p>
      <p className='pointer-events-none block text-xs text-gray-500'>{file.size}</p>
    </li>
  )
}
// function PhotoList() {
...
function PhotoList() {
  return (
    <ul
      role='list'
      className='grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-3 lg:grid-cols-4 xl:gap-x-10'
    >
      {files.map((file) => (
...
export default function App() {
  return (
    <div className='bg-purple-100 min-h-screen'>
      <div className='mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8'>
        <h1 className='text-2xl font-bold text-gray-800 mb-6'>Fotoğraf Galerisi</h1>
        <PhotoList />
      </div>
    </div>
  )
